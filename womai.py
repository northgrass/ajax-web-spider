...
file = open('womai-homework.txt', 'a+')
file.write('商品id\t商品名\t价格\t评价数\n')
...
while next_page:
    ...
    comment_counts = tree.xpath("//div[@class='rated']/span[@class='evaluated']/a/em/text()")
    commentAll += comment_counts
...
for i in range(len(idsAll)):
    out = idsAll[i].strip() + '\t' + namesAll[i].strip() + '\t' + priceAll[i].strip() + '\t' + commentAll[i]
